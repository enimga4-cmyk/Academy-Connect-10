...
export default function App() {
  ...
  const handleDeleteNote = async (
    ...
    const storagePath = noteToDelete.storagePath || noteToDelete.pdfUrl;

    console.log(`[Storage Deletion Log] Attempting to delete note PDF. Bucket: ${bucket}, StoragePath: ${storagePath}`);

    if (storagePath && !storagePath.startsWith("data:") && !storagePath.startsWith("http://") && !storagePath.startsWith("https://")) {
      try {
        const { data, error } = await supabase.storage.from(bucket).remove([storagePath]);
        console.log(`[Storage Deletion Log] Response:`, data);
        
        if (error) {
          console.error(`[Storage Deletion Log] Error:`, error);
          throw new Error(`Supabase Storage deletion failed: ${error.message || error}`);
        }
      } catch (storageErr: any) {
        console.error(`[Storage Deletion Log] Deletion failed completely:`, storageErr);
        throw new Error(`Failed to delete PDF from storage. Database record was preserved. Error: ${storageErr.message || storageErr}`);
      }
    } else {
      console.log(`[Storage Deletion Log] Note uses offline/base64 fallback or has no valid storagePath.`);
    }

    // Only after successful deletion, remove the Firestore/database document
    const updated = {
      ...targetStudent,
      notes: {
        ...targetStudent.notes,
    ...
    try {
      ...
      console.log(`[Storage Deletion Log] Successfully removed note from Firestore.`);
    } catch (dbErr: any) {
      console.error(`[Storage Deletion Log] Failed to update Firestore database document:`, dbErr);
      throw new Error(`PDF was deleted from storage, but database update failed: ${dbErr.message || dbErr}`);
    }
  };
