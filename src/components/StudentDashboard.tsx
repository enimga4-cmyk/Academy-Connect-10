...
export function generateSubjectPdfReport(student: Student, subject: string, notes: ChapterNote[]) {
  ...
  doc.text(`App Version: 3.5.0`, 14, 34);
...
export function StudentMyTab({ 
  ...
  const handleDownloadPdf = async (note: ChapterNote) => {
    ...
    const storagePath = note.storagePath || note.pdfUrl;

    try {
      setDownloadProgress(30);
      
      // Download the file directly from Supabase Storage using the official SDK download method
      const { data: blob, error } = await supabase.storage.from(bucket).download(storagePath);
      if (error) throw error;
      if (!blob) throw new Error("No data returned from storage.");
      ...
      console.error("[StudentDashboard] Supabase download failed. Attempting signed URL redirect fallback.", downloadError);
      
      // Fallback: generate a signed URL and open/download it
      try {
        setDownloadProgress(50);
        const { data, error } = await supabase.storage.from(bucket).createSignedUrl(storagePath, 60);
        if (error) throw error;
        
        const resolvedUrl = data?.signedUrl;
        if (!resolvedUrl) throw new Error("Could not resolve signed URL.");
        ...
        console.error("[StudentDashboard] All download methods failed:", fallbackError);
        alert(`Could not download file: ${fallbackError.message || fallbackError}`);
      }
    } finally {
      setDownloadingNoteId(null);
      setDownloadProgress(0);
                    ...
                    <div key={note.id} className="rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50/40 dark:bg-slate-950/20 p-3 hover:border-slate-200 dark:hover:border-slate-750 hover:bg-slate-50/80 dark:hover:bg-slate-950/30 transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      ...
                      <div className="flex flex-wrap items-center gap-1.5 self-end sm:self-center">
                        ...
                        {downloadingNoteId === note.id ? (
                          <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-400 text-[10px] font-bold border border-emerald-100 dark:border-emerald-900/50">
